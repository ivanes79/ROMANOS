...
def test_romano_a_entero_no_repetir_mas_de_tres_caracteres_especiales():
    with pytest.raises( RomanNumberError ) as exceptionInfo:
        romano_a_entero("DD")    
    assert str(exceptionInfo.value) == "No se puede repetir estos valores: L,D,V, su valor repetido es D"      
...
def test_romano_a_entero_reglas_de_restas():
    with pytest.raises( RomanNumberError ) as exceptionInfo:
        romano_a_entero("C")    
    assert str(exceptionInfo.value) == "El simbolo romano C solo se puede restar de D y M"
"""
